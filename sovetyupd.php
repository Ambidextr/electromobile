	...
	<div id="pagin">
		...
		<form method="get" name="upd">
            ...
            <?php
				...
				{
                    echo "<option value=".$row['id_sovety'].">".$row['sovety_name']."</option>";
                }
            ?>
             </select>
...
$q="SELECT * FROM sovety WHERE id_sovety = " .$_GET['update'];
...
if (mysql_num_rows($result)>0) //Проверка кол-ва записей
	...
	if ($row = mysql_fetch_assoc($result))
		...
		$sql_autor_sovety=$row["sovety_autor"];
...
<label>
Дата создания:</br>
<input type="date" name="date" value=<?php echo ''.$sql_data_sovety.'';?> required>
</br>
Изображение:</br>
...
<?php echo ''.$sql_name_sovety.'';?>
</textarea></br>
Текст:</br>
<textarea name="text" style="height: 300px; width: 500px" maxlength="15000" required>
<?php echo ''.$sql_text_sovety.'';?>
</textarea></br>
</label>
<input name="id" type="hidden" id="id" value=<?php echo ''.$sql_id_sovety.'';?>>
...
{
$upd_query = mysql_query("UPDATE sovety SET sovety_name='".$_POST['name_news']."', sovety_text='".$_POST['text']."', sovety_picture='".$_FILES["filename"]["name"]."', sovety_data='".$_POST['date']."', auto_kategory='".$_POST['auto']."' WHERE id_sovety='".$_POST['id']."'") or die(mysql_error());
        if ($upd_query) 
		{
...
{
$upd_query = mysql_query("UPDATE sovety SET sovety_name='".$_POST['name_news']."', sovety_text='".$_POST['text']."', sovety_data='".$_POST['date']."', auto_kategory='".$_POST['auto']."' WHERE id_sovety='".$_POST['id']."'") or die(mysql_error());
        if ($upd_query) 
		{
			echo "Обновление прошло успешно";
        }
