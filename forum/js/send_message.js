...
bbcode = new Array();
bbtags = new Array('[b]','[/b]','[i]','[/i]','[u]','[/u]','[quote]','[/quote]','[code]','[/code]','[php]','[/php]','[list]','[/list]','[list=]','[/list]','[img]','[/img]','[url]','[/url]');
imageTag = false;

// Shows the help messages in the helpline window
function helpline(help) {
	document.getElementById("sendMessageForm").helpbox.value = eval(help + "_help");
}
...
function checkForm() {
	...
	if (document.getElementById("sendMessageForm").message.value.length < 2) {
...
function emoticon(text) {
	var txtarea = document.getElementById("sendMessageForm").message;
	text = ' ' + text + ' ';
...
function bbstyle(bbnumber) {
	...
	if (bbnumber == -1) { // Close all open tags & default button names
		while (bbcode[0]) {
			...
			buttext = eval('document.getElementById("sendMessageForm").addbbcode' + butnumber + '.value');
			eval('document.getElementById("sendMessageForm").addbbcode' + butnumber + '.value ="' + buttext.substr(0,(buttext.length - 1)) + '"');
		}
	...
	if (donotinsert) {		// Close all open tags up to the one just clicked & default button names
		...
		if (imageTag && (bbnumber != 16)) {		// Close image tag before adding another
			...
			document.getElementById("sendMessageForm").addbbcode16.value = "Img";	// Return button back to normal state
			imageTag = false;
		}
		...
		eval('document.getElementById("sendMessageForm").addbbcode'+bbnumber+'.value += "*"');
		txtarea.focus();
		return;
	}
	storeCaret(txtarea);
